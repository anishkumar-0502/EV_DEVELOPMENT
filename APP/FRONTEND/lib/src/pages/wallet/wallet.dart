...
class _WalletPageState extends State<WalletPage> {
  ...
  void showRechargeModel(double walletBalance, Function fetchWallet,
        ...
        children: [
          ...
          if (!isDataLoaded)
            ...
            _buildSection(
              ...
              '1. Click on the "Recharge Now" button to open the recharge dialog.\n'
                  '2. In the dialog, enter the amount you want to add to your wallet. The amount must not exceed the wallet limit (₹10,000).\n'
                  '3. Use the quick buttons (₹500, ₹1000, Maximum) to quickly add predefined amounts to the input field.\n'
                  '4. Click on "Continue" to proceed with the payment.\n'
                  '5. Once the payment is successful, the wallet balance will be updated.',
            ),
            const SizedBox(height: 16),
            _buildSection(
              'Need More Help?',
              'For further assistance, contact our support team at info@outdidunified.com.',
            ),
          ]
        ],
      ),
  ...
  void _handlePaymentSuccess(PaymentSuccessResponse response) async {
    ...
    try {
      ...
      if (responseData == 1) {
        ...
        widget
            .fetchWallet(); // Call the fetchWallet() method passed from the parent
...
// Show the loading spinner
      ...
      if (amount > 0) {
        double totalAmount = widget.walletBalance + amount;

        // Check if the total amount exceeds the maximum allowed limit (₹10,000)
        if (totalAmount <= 10000) {
          try {
            handlePayment(amount); // Call your payment function (must be async)
            setState(() {
// Stop the loading spinner on success
            });
          } catch (e) {
              ...
              children: [
                ...
                TextField(
                  ...
                  inputFormatters: [
                    ...
                    LengthLimitingTextInputFormatter(
                        7), // Limit input to 7 characters (4 digits + decimal + 2 digits)
                  ],
                  onChanged: (value) {
                    setState(() {
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: showAlertLoading
                        ? null
                        : onSubmit, // Disable the button when loading
                    style: ElevatedButton.styleFrom(
                      padding: EdgeInsets.symmetric(
                      ...
                      elevation: 8,
                      backgroundColor: showAlertLoading
                          ? Colors
                              .grey.shade400 // Change button color when loading
                          : Colors.transparent,
                    ).copyWith(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        ...
                        color: showAlertLoading
                            ? Colors.grey.shade400
                            : null, // Grey color for loading
                        borderRadius: BorderRadius.circular(
                          showAlertLoading ? 30 : 10, // Dynamic border radius
                        ),
                      ),
                      alignment: Alignment.center,
                      child: showAlertLoading
                          ? const SizedBox(
                              width: 25, // Smaller spinner size
                              height: 25,
