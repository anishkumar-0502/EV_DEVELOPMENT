...
class _ChargingPageState extends State<Charging>
  ...
  void setApiData(
      Map<String, dynamic> chargerSession, Map<String, dynamic> userValue) {
    setState(() {
  ...
  Future<void> updateSessionPriceToUser(int? connectorId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        double walletBal = (updatedUser['wallet_bal'] ?? 0).toDouble();

        setState(() {
          walletBalance = walletBal; // Update wallet balance
        });

        _initializeNotification(chargerStatus);
  ...
  Future<void> _initializeNotification(String chargerStatus) async {
    try {
      ...
      // Notification 1: Conditional Notification
      if (walletBalance < 100 && !_isNotificatonSent) {
        try {
          await _notificationService.showNotification(
            id: 1, // Unique ID for this notification
            title: 'Wallet Amount Alert!',
            body: 'Your Wallet Amount balance is below Rs.100.',
            payload: 'low_balance_alert',
          );
          // Set the flag to avoid sending duplicate notifications
          _isNotificatonSent = true;
          print('Conditional Notification sent');
        } catch (e) {
          print('Error sending Wallet Amount Alert notification: $e');
        }
      }

      // Notification 2: Unconditional Notification
      if (errorCode == 'NoError' && !_isNotificatonSent) {
        try {
          await _notificationService.showNotification(
            id: 2, // Unique ID for this notification
            title: 'Charging Completed!',
            body: 'Ready to go! Have a safe journey ahead!',
            payload: 'charging_complete_notification',
          );
          print('Unconditional Notification sent');
        } catch (e) {
          print('Error sending Charging Completed notification: $e');
        }
        ...
        try {
          await _notificationService.showNotification(
            ...
            body:
                'Charging process was interrupted due to a fault. Please check the charger.',
            payload: 'fault_notification',
          );
          print('Fault Notification sent');
        } catch (e) {
          print('Error sending Fault Detected notification: $e');
        }
      }
    } catch (e) {
  ...
  void RcdMsg(Map<String, dynamic> parsedMessage) async {
    ...
    if (message[2] == 'MeterValues') {
      ...
      if (meterValues.isNotEmpty && !isBatteryScreenVisible) {
        ...
        setState(() {
          showMeterValuesContainer =
              voltageV1.isNotEmpty || currentA1.isNotEmpty;
    ...
    if (parsedMessage['DeviceID'] == chargerID &&
        connectorIds == widget.connector_id &&
        msg.isNotEmpty) {
      ...
      switch (message[2]) {
        case 'StatusNotification':
          ...
          currentTime = formatTimestamp(DateTime.tryParse(
                  message[3]['timestamp'] ?? DateTime.now().toString()) ??
              DateTime.now());
        ...
        case 'MeterValues':
          if (!showMeterValuesContainer) {
            ...
            print(meterValues);
            final sampledValue =
                meterValues.isNotEmpty ? meterValues[0]['sampledValue'] : [];

            Map<String, dynamic> formattedJson =
                convertToFormattedJson(sampledValue);
            currentTime = formatTimestamp(DateTime.now());
            ...
            print(
                '{ "V": ${formattedJson['Voltage']},"A": ${formattedJson['Current.Import']},"W": ${formattedJson['Power.Active.Import']},"Wh": ${formattedJson['Energy.Active.Import.Register']},"Hz": ${formattedJson['Frequency']},"Kelvin": ${formattedJson['Temperature']}}');
          }
          break;

        case 'Authorize':
          print("errorCode: $errorCode");
          chargerStatus = (errorCode == 'NoError' || errorCode.isEmpty)
              ? 'Authorize'
              : 'Faulted';
          currentTime = formatTimestamp(DateTime.now());
          break;
        ...
        if (isLoading) {
          ...
          setState(() {
            ...
            NoResponseFromCharger =
                true; // Set the flag to show the alert banner
          });
        ...
        return Card(
          ...
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(isSmallScreen
                ? 10
                : 15), // Adjust border radius for small screens
          ),
          child: Container(
            ...
            padding: EdgeInsets.all(
                isSmallScreen ? 12 : 16.0), // Adjust padding for small screens
            ...
            child: Row(
              ...
              children: [
                ...
                const SizedBox(
                    width:
                        20), // Add some space between the circle and the text
                Container(
                  width: isSmallScreen
                      ? 80
                      : 90, // Adjust circle size for small screens
  ...
  void thresholdlevel() {
    ...
    String overCurrent =
        chargerCapacity == "3.5" ? 'Over Current - 17A' : 'Over Current - 33A';
    print("chargerCapacity $chargerCapacity");
        ...
        return Padding(
          ...
          child: Container(
            ...
            height: MediaQuery.of(context).size.height *
                0.7, // Adjusted height for better fit
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ...
                Expanded(
                  child: SingleChildScrollView(
                    child: Center(
                      child: Container(
                        width: double
                            .infinity, // Make container take up full width
                        padding: const EdgeInsets.all(16.0),
                        decoration: BoxDecoration(
                          color: Colors.black,
    ...
    showModalBottomSheet(
      ...
      builder: (BuildContext context) {
        return Container(
            ...
            child:
                ErrorDialog(isErrorVisible: isErrorVisible, history: history));
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        body: LoadingOverlay(
          showAlertLoading:
              showAlertLoading || isLoading, // Combine both loading states
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: <Widget>[
              Padding(
                padding: EdgeInsets.only(
                  top: isSmallScreen
                      ? 30.0
                      : 40.0, // Adjust top padding based on screen size
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    GestureDetector(
                      ...
                      child: Padding(
                        ...
                        child: Icon(
                          ...
                          size: isSmallScreen
                              ? 25
                              : 30, // Adjust size based on screen size
                        ),
                      ),
                    ),
                    ...
                    Padding(
                      padding: EdgeInsets.only(
                          top: isSmallScreen ? 5 : 0), // Adjust top padding
                      child: IconButton(
                        onPressed: chargerStopSettings,
                        icon: const Icon(Icons.settings, color: Colors.white),
                        iconSize: isSmallScreen
                            ? 22
                            : 24, // Adjust size based on screen size
                      ),
                    ),
                    // Power Icon
                    Padding(
                  ...
                  child: Center(
                    child: Column(
                      children: [
                        Padding(
                          padding: EdgeInsets.symmetric(
                            horizontal: isSmallScreen
                                ? 12.0
                                : 20.0, // Adjust padding based on screen size
                          ),
                          child: Container(
                            height: isSmallScreen
                                ? 50
                                : 65, // Adjust height for smaller screens
                            width: screenWidth *
                                0.9, // Adjust width based on screen size (90% of screen width)
                            decoration: BoxDecoration(
                              color: const Color(0xFF1E1E1E),
                              ...
                              children: [
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      ...
                                      style: TextStyle(
                                        ...
                                        fontSize: isSmallScreen
                                            ? 16
                                            : 20, // Adjust text size for small screens
                                        fontWeight: FontWeight.bold,
                                      ),
                                    ),
                                  ],
                                ...
                                const Text(
                                  ...
                                  style: TextStyle(
                                    fontSize: 30,
                                    color: Colors
                                        .white, // Adjust the color for visibility
                                  ),
                                ),
                                Row(
                                  children: [
                        ...
                        Padding(
                          padding: EdgeInsets.only(
                              left: isSmallScreen ? 8.0 : 13.0,
                              bottom: isSmallScreen ? 10 : 15),
                          child: Padding(
                            padding:
                                EdgeInsets.all(isSmallScreen ? 12.0 : 18.0),
                            child: Row(
                              children: [
                            ...
                            children: [
                              if (chargerStatus == 'Preparing')
                                ...
                                StopButton(
                                  ...
                                  isStopLoading:
                                      _isStopLoading, // Pass the loading state here
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(
            isSmallScreen ? 10 : 15), // Adjust border radius for small screens
      ),
      child: Padding(
        padding: EdgeInsets.all(
            isSmallScreen ? 15 : 20.0), // Adjust padding for small screens
          ...
          child: Padding(
            ...
            child: Container(
              ...
              child: IconButton(
                ...
                onPressed: widget.chargerStatus == 'Preparing' &&
                        widget.isStartButtonEnabled
                    ? widget.onPressed
                    : null,
              ),
            ),
        ...
        child: AnimatedContainer(
          ...
          child: Padding(
            ...
            child: Container(
              ...
              child: IconButton(
                icon:
                    const Icon(Icons.power_off, color: Colors.white, size: 32),
                onPressed: () {
...
class ErrorDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title:
            const Text('Error History', style: TextStyle(color: Colors.white)),
        backgroundColor: Colors.black,
                      ...
                      child: Container(
                        ...
                        child: Scrollbar(
                          child: ListView.builder(
                            shrinkWrap:
                                true, // This ensures the ListView takes only the required space
...
class ChargingCompleteModal extends StatefulWidget {
  ...
  @override
  State<ChargingCompleteModal> createState() => _ChargingCompleteModalState();
}

class _ChargingCompleteModalState extends State<ChargingCompleteModal> {
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                ...
                subtitle: Text(
                  '${widget.chargingSession['charger_id']}',
              ...
              ListTile(
                leading: CircleAvatar(
                  backgroundColor: Colors.grey.shade600,
                  child:
                      const Icon(Icons.numbers, color: Colors.white, size: 24),
                ),
                title: const Text(
                  'Connector Id',
                  style: TextStyle(fontSize: 18, color: Colors.white),
                ...
                subtitle: Text(
                  '${widget.chargingSession['connector_id']}',
                  style: const TextStyle(fontSize: 16, color: Colors.white70),
                ),
              ),
              const SizedBox(height: 8),
              ListTile(
                ...
                subtitle: Text(
                  _getConnectorTypeName(
                      widget.chargingSession['connector_type']),
                  style: const TextStyle(fontSize: 16, color: Colors.white70),
                ),
              ),
              const SizedBox(height: 8),
              ListTile(
                ...
                subtitle: Text(
                  widget.chargingSession['start_time'] != null
                      ? formatTimestamp(widget.chargingSession['start_time'])
                      : "N/A",
                  style: const TextStyle(fontSize: 16, color: Colors.white70),
                ),
              ),
              ...
              ListTile(
                ...
                subtitle: Text(
                  widget.chargingSession['stop_time'] != null
                      ? formatTimestamp(widget.chargingSession['stop_time'])
                      : "N/A",
                  style: const TextStyle(fontSize: 16, color: Colors.white70),
                ),
              ),
              ...
              ListTile(
                ...
                subtitle: Text(
                  '${widget.chargingSession['unit_consummed']} Kwh',
                  style: const TextStyle(fontSize: 16, color: Colors.white70),
                ),
              ),
              const SizedBox(height: 8),
              ListTile(
                ...
                subtitle: Text(
                  'Rs. ${widget.chargingSession['price']}',
                  style: const TextStyle(fontSize: 16, color: Colors.white70),
                ),
              ),
              const SizedBox(height: 8),
