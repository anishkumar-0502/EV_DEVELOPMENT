import 'dart:async';
import 'dart:convert';
import 'package:ev_app/src/pages/profile/Bluetooth/read_page.dart';
import 'package:ev_app/src/pages/profile/Bluetooth/write_page.dart';
...
class _DeviceDetailsPageState extends State<DeviceDetailsPage> {
  ...
  void _logMessage(String message, {Color? color}) {
    ...
    setState(() {
      ...
      if (color != null) {
        ...
        _logColors
            .add(Colors.white70); // Use default color if no color is provided
      }
    });
  ...
  Future<void> _checkConnectionStatus() async {
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        if (_connection != null && _connection!.isConnected) {
          ...
          _logMessage("Connected to ${widget.device.name ?? 'Unknown Device'}",
              color: Colors.yellow);
          _startReadOperation();
    ...
    _logMessage("Failed to connect after $maxRetries attempts.",
        color: Colors.red);
  }
  ...
  Future<void> _toggleConnection() async {
    if (_isConnected) {
      ...
      _logMessage("Disconnected from ${widget.device.name ?? 'Unknown Device'}",
          color: Colors.yellow);
    } else {
      _logMessage(
  ...
  Future<void> writeDataAndVerify() async {
    try {
      ...
      setState(() {
        ...
        _fabColor = Colors.green; // Set the color to green for success
      });
      print("formattedData:${formattedData}");
      // Add to logs
      _logMessage("$formattedData", color: Colors.lightBlueAccent);
      if (_connection != null && _connection!.isConnected) {
              ...
              children: [
                ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: const Color(0xFF1E1E1E).withOpacity(1.0),
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                          ...
                          focusedBorder: UnderlineInputBorder(
                            borderSide: BorderSide(
                                color: Colors
                                    .grey), // Bottom border color when focused
                          )),
                    ),
                  ),
                  SizedBox(width: 8),
